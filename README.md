...
- 👑 **角色权限管理** - 区分管理员和普通用户，系统限制只能有一个管理员
- 🚀 **实例管理** - 启动、停止、重启 SillyTavern 实例（仅普通用户）
- 📊 **资源监控** - 实时查看 CPU、内存、运行时间等
- 📦 **版本管理** - 切换版本、重装依赖、依赖检测
- 📄 **实时日志** - 查看标准输出和错误日志，支持自动刷新
- 🔧 **管理员面板** - 管理所有用户、实例控制、系统统计（管理员专用）
- 🌐 **Nginx 反向代理** - 支持路径转发（如：`/用户名/st`），统一域名访问
- 🔒 **数据隔离** - 每个普通用户拥有独立的数据目录和 SillyTavern 实例
- 💾 **数据备份** - 支持将数据备份到 Hugging Face 仓库（新增）
- 🎨 **现代化UI** - 简洁美观的管理界面
- ⚡ **自动化部署** - 基于 PM2 的进程管理
...
> **📌 重要说明**：管理员账户仅用于管理系统和其他用户，不会创建 SillyTavern 实例。管理员登录后将直接进入管理员面板。

### 管理员面板功能

管理员登录后会自动进入管理员面板（如果您需要同时管理和使用 SillyTavern，请创建两个账户：一个管理员账户和一个普通用户账户）。
...
- 用户实例：`http://localhost:3001`, `http://localhost:3002` ...

### 模式 2: Nginx 路径转发（推荐）
- 管理平台：`http://yourdomain.com`
- 用户实例：`http://yourdomain.com/username/st`

**优势：** 统一域名、单端口、更安全、支持 HTTPS

详细配置请参考 [NGINX-SETUP.md](./NGINX-SETUP.md)
...
用户可以将 SillyTavern 数据备份到 Hugging Face：

1. 在控制台找到"💾 数据备份"卡片
2. 配置 Hugging Face Token 和仓库名
3. 测试连接确保配置正确
4. 点击"立即备份"执行备份

详细说明请查看 [HUGGINGFACE-BACKUP.md](./HUGGINGFACE-BACKUP.md)
...
2. **使用 HTTPS** - 在生产环境中使用 HTTPS（推荐通过 Nginx + Let's Encrypt）
3. **使用 Nginx 反向代理** - 推荐使用 Nginx 模式，只对外开放 80/443 端口，内部端口不对外暴露
4. **防火墙配置** - 如使用直接端口模式，仅开放必要的端口（3000-4000）
5. **保护管理员账户** - 使用强密码，至少创建一个管理员账户作为备份
6. **定期备份** - 定期备份 `database.sqlite` 和 `data/` 目录
7. **限制注册** - 根据需要添加注册限制（如邮箱验证、邀请码等）
8. **安装 Git** - 服务器需要安装 Git 才能克隆 SillyTavern 仓库
9. **磁盘空间** - 确保有足够的磁盘空间（每个用户约 500MB-1GB）
10. **权限审计** - 定期检查管理员账户列表，确保只有授权人员拥有管理员权限
